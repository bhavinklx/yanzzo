                ...
                <div class="col-md-7 align-self-center text-right">
                    <div class="d-flex justify-content-end align-items-center">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Usp List</li>
                        </ol>
                        @if(auth()->user()->can('usp-add'))
                            <a href="{{ route("usp-add") }}" class="btn-sm btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> Create New</a>
                        @endif
...
@section('page-js')
    <script type="text/javascript">
        ...
        function change_status(usp_id, status) {
            $.ajax({
                url: "{{ route('usp-change-status') }}",
                method: "POST",
                data: {
                    usp_id:usp_id,
                    status:status,
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_status_"+usp_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+usp_id+"', '0')\" ><div class=\"label label-table label-success\">Active</div></a>");
                    } else {
                        $("#td_status_"+usp_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+usp_id+"', '1')\" ><div class=\"label label-table label-danger\">Inactive</div></a>");
                    }
                }
            });
        }

        function deleteData(usp_id) {
            $.ajax({
                url: "{{ route('usp-delete') }}",
                type: "POST",
                data: {
                    usp_id:usp_id,
                    _token:'{{ csrf_token() }}'
