            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h5 class="card-title">Sponsor List</h5>
                        @if (auth()->user()->can('sponsor-add'))
                            <a href="{{ route("sponsor-add") }}" class="btn btn-primary ms-auto">Add Sponsor</a>
                        @endif
                    </div>
                    <div class="card-body">
                        <!-- Table starts -->
...
@section('page-js')
    <script type="text/javascript">
        ...
        var table = $('#basicExample').DataTable({
            ...
            ordering: true,
            autoWidth: false,
            ajax: '{{ route("sponsor-load-table") }}',
            columns: [
                { data: 'checkbox', orderable: false, searchable: false },
                { data: 'title', name: 'sponsor_title' },
        ...
        function change_status(sponsor_id, status) {
            $.ajax({
                url: "{{ route('sponsor-change-status') }}",
                method: "POST",
                data: {
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_status_"+sponsor_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+sponsor_id+"', '0')\" ><span class=\"badge bg-success\">Active</span></a>");
                    } else {
                        $("#td_status_"+sponsor_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+sponsor_id+"', '1')\" ><span class=\"badge bg-danger\">Inactive</span></a>");
                    }
                }
            });
        }

        function openDeleteModal(sponsor_id) {
        ...
        function deleteData() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    /*$('#row_' + sponsor_id).remove();
                     setTimeout(function(){
                     location.reload();
                     },2000);*/
                    table.row($('#row_' + sponsor_id))
                            .remove()
                            .draw(false);
                }
            });
