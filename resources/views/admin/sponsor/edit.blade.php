...
@section('content')
    ...
    <!-- App body starts -->
    <form id="sponsorFrm" method="post" action="{{ route('sponsor-update') }}">
        <input type="hidden" name="sponsor_id" value="{{ $sponsorDetail->sponsor_id }}">
        {{ csrf_field() }}
        <div class="app-body">
            <!-- Row starts -->
            <div class="row gx-3">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-xxl-12 col-lg-4 col-sm-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" id="sponsor_title" name="sponsor_title" placeholder="Enter Sponsor Title" value="{{ $sponsorDetail->sponsor_title }}">
                                        ...
                                        <div class="col-xxl-12 col-lg-4 col-sm-6" style="margin-top: 2rem !important">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="sponsor_status" name="sponsor_status">
                                                    <option value="1"
                                                            {{ $sponsorDetail->sponsor_status == '1' ? 'selected' : '' }}>Active</option>
                                                    <option value="0"
                                                            {{ $sponsorDetail->sponsor_status == '0' ? 'selected' : '' }}>Inactive
                                                    </option>
                                                </select>
                                            </div>
                                        </div>
...
@section('page-js')
    <script type="text/javascript">
        ...
        let existingImage = "{{ $sponsorDetail->sponsor_image ?? '' }}";
            ...
            init: function() {
                ...
                if (existingImage) {
                    ...
                    myDropzone.emit(
                            "thumbnail",
                            mockFile,
                            "{{ asset('uploads/sponsor') }}/" + existingImage
                    );
                    myDropzone.emit("complete", mockFile);

                    myDropzone.files.push(mockFile);
                    document.getElementById('sponsor_image').value = existingImage;
                }
