            ...
            <div class="row page-titles">
                ...
                <div class="col-md-7 align-self-center text-right">
                    <div class="d-flex justify-content-end align-items-center">
                        ...
                        @can('testimonial-add')
                            <a href="{{ route("testimonial-add") }}" class="btn-sm btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> Create New</a>
                        @endcan
...
@section('page-js')
    <script type="text/javascript">
        ...
        $('#myTable1').DataTable({
            ...
            order: [],
            paging: true,
            ajax: '{{ route("testimonial-load-table") }}',
            columns: [
                { data: 'checkbox' },
                { data: 'title' },
        ...
        function sendOrderToServer() {
            ...
            $.ajax({
                url: "{{ route('testimonial-update-order') }}",
                method: "POST",
                data:{
                    order:order,
                    _token: '{{csrf_token()}}'
        ...
        function change_status(testimonial_id, status) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_status_"+testimonial_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+testimonial_id+"', '0')\" ><div class=\"label label-table label-success\">Active</div></a>");
                    } else {
                        $("#td_status_"+testimonial_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+testimonial_id+"', '1')\" ><div class=\"label label-table label-danger\">Inactive</div></a>");
                    }
                }
            });
        }

        function deleteData(testimonial_id) {
            $.ajax({
