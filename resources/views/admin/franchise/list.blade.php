                ...
                <div class="col-md-7 align-self-center text-right">
                    <div class="d-flex justify-content-end align-items-center">
                        ...
                        @if(auth()->user()->can('franchise-add'))
                            <a href="{{ route("franchise-add") }}" class="btn-sm btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> Create New</a>
                        @endif
...
@section('page-js')
    <script type="text/javascript">
        ...
        $('#myTable1').DataTable({
            ...
            order: [],
            paging: true,
            ajax: '{{ route("franchise-load-table") }}',
            columns: [
                { data: 'checkbox' },
                { data: 'company_name' },
                { data: 'title' },
        ...
        function sendOrderToServer() {
            ...
            $('tr.row1').each(function(index, element) {
                order.push({
                    franchise_id: $(this).attr('data-id'),
                    position: index + 1
                });
            });

            $.ajax({
                url: "{{ route('franchise-update-order') }}",
                method: "POST",
                data:{
                    order:order,
                    _token: '{{csrf_token()}}'
        ...
        function change_status(franchise_id, status) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_status_"+franchise_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+franchise_id+"', '0')\" ><div class=\"label label-table label-success\">Active</div></a>");
                    } else {
                        $("#td_status_"+franchise_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+franchise_id+"', '1')\" ><div class=\"label label-table label-danger\">Inactive</div></a>");
                    }
                }
            });
        }

        function deleteData(franchise_id) {
            $.ajax({
                url: "{{ route('franchise-delete') }}",
                type: "POST",
                data: {
                    franchise_id:franchise_id,
                    _token:'{{ csrf_token() }}'
