                ...
                <div class="col-md-7 align-self-center text-right">
                    <div class="d-flex justify-content-end align-items-center">
                        ...
                        @if(auth()->user()->can('city-add'))
                            <a href="{{ route("city-add") }}" class="btn-sm btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> Create New</a>
                        @endif
...
@section('page-js')
    <script type="text/javascript">
        ...
        function change_status(city_id, status) {
            $.ajax({
                url: "{{ route('city-change-status') }}",
                method: "POST",
                data: {
                    city_id:city_id,
                    status:status,
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_status_"+city_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+city_id+"', '0')\" ><div class=\"label label-table label-success\">Active</div></a>");
                    } else {
                        $("#td_status_"+city_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+city_id+"', '1')\" ><div class=\"label label-table label-danger\">Inactive</div></a>");
                    }
                }
            });
        }

        function deleteData(city_id) {
            $.ajax({
