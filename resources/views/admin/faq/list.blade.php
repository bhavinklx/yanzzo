                ...
                <div class="col-md-7 align-self-center text-right">
                    <div class="d-flex justify-content-end align-items-center">
                        ...
                        @if(auth()->user()->can('faq-add'))
                            <a href="{{ route("faq-add") }}" class="btn-sm btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> Create New</a>
                        @endif
...
@section('page-js')
    <script type="text/javascript">
        ...
        function change_status(faq_id, status) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_status_"+faq_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+faq_id+"', '0')\" ><div class=\"label label-table label-success\">Active</div></a>");
                    } else {
                        $("#td_status_"+faq_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+faq_id+"', '1')\" ><div class=\"label label-table label-danger\">Inactive</div></a>");
                    }
                }
            });
        }

        function change_home_status(faq_id, status) {
            $.ajax({
                url: "{{ route('faq-change-home-status') }}",
                method: "POST",
                data: {
                    faq_id:faq_id,
                    status:status,
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_home_status_"+faq_id).html("<a href=\"javascript:void(0)\" onclick=\"change_home_status('"+faq_id+"', '0')\" ><div class=\"label label-table label-success\">Active</div></a>");
                    } else {
                        $("#td_home_status_"+faq_id).html("<a href=\"javascript:void(0)\" onclick=\"change_home_status('"+faq_id+"', '1')\" ><div class=\"label label-table label-danger\">Inactive</div></a>");
                    }
                }
            });
        }

        function deleteData(faq_id) {
            $.ajax({
