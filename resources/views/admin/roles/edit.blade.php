...
@section('content')
    ...
    <div class="page-wrapper">
        ...
        <div class="container-fluid">
            ...
            <!-- Start Page Content -->
            <form class="floating-labels" id="roleFrm" method="post" action="{{ route('role-update') }}" >
                                                        ...
                                                        @if ($permission && count($permission) > 0)
                                                            @foreach ($permission as $key => $value)
                                                                <div class="custom-control custom-checkbox col-lg-2 p-10 m-b-15 {{ ($key < 6) ? 'm-t-10' : '' }}">
                                                                    <input type="checkbox" class="custom-control-input" id="permission{{ $value->id }}" name="permission[]" value="{{ $value->id }}" {{ in_array($value->id, $rolePermissions) ? 'checked="checked"' : '' }} />
