                ...
                <div class="col-md-7 align-self-center text-right">
                    <div class="d-flex justify-content-end align-items-center">
                        ...
                        @if(auth()->user()->can('blog-add'))
                            <a href="{{ route("blog-add") }}" class="btn-sm btn-info d-none d-lg-block m-l-15"><i class="fa fa-plus-circle"></i> Create New</a>
                        @endif
        ...
        function change_status(blog_id, status) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_status_"+blog_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+blog_id+"', '0')\" ><div class=\"label label-table label-success\">Active</div></a>");
                    } else {
                        $("#td_status_"+blog_id).html("<a href=\"javascript:void(0)\" onclick=\"change_status('"+blog_id+"', '1')\" ><div class=\"label label-table label-danger\">Inactive</div></a>");
                    }
                }
            });
        }

        function change_popular_status(blog_id, status) {
            $.ajax({
                url: "{{ route('blog-change-popular-status') }}",
                method: "POST",
                data: {
                    blog_id:blog_id,
                    status:status,
                ...
                success: function (response) {
                    ...
                    if (status == 1){
                        $("#td_popular_status_"+blog_id).html("<a href=\"javascript:void(0)\" onclick=\"change_popular_status('"+blog_id+"', '0')\" ><div class=\"label label-table label-success\">Yes</div></a>");
                    } else {
                        $("#td_popular_status_"+blog_id).html("<a href=\"javascript:void(0)\" onclick=\"change_popular_status('"+blog_id+"', '1')\" ><div class=\"label label-table label-danger\">No</div></a>");
                    }
                }
            });
        }
